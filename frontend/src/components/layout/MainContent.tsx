...
const PAGE_META: Record<PageId, { tag: string; tagColor: string; tagBg: string; title: string; desc: string; isOptima?: boolean }> = {
  ...
  provider:        { tag:'Platform Admin',        tagColor:'#7C3AED', tagBg:'#EDE9FE', title:'Provider Admin',              desc:'Register provider organisations and manage the default PROVIDER_ADMIN user.' },
  ...
  tenant:          { tag:'Platform Admin',        tagColor:'#0891B2', tagBg:'#E0F7FA', title:'Tenant Admin',                desc:'Tenant Admin roster. Invite Tenant User opens a requirement form per tenant; profiles stay PENDING until Provider Admin approves. Edit available for Provider Admin.' },
  ...
  'tenant-admin-portal': { tag:'Tenant Admin',    tagColor:'#0D9488', tagBg:'#CCFBF1', title:'Tenant Admin Portal',         desc:'Your registration plus Invite Tenant User requirement form per tenant. Submitted profiles stay PENDING until Provider Admin approves.' },
  ...
  cost:            { tag:'Stage 3',              tagColor:'#0891B2', tagBg:'#E0F7FA', title:'Cost Estimation & Review',    desc:'Edit resource costs inline. Budget ceiling gate enforced. Over-budget deployments route to Tenant Admin approval queue.' },
  terraform:       { tag:'Stage 4',              tagColor:'#2563EB', tagBg:'#EFF6FF', title:'Terraform HCL Generation',    desc:'LLM generates 4 production-ready Terraform files. OPA, tfsec, and Checkov scan applied. Artifact stored in S3. NFR: under 30 seconds.' },
  jumpbox:         { tag:'Stage 5',              tagColor:'#059669', tagBg:'#D1FAE5', title:'Execution Engine (Jump Box)', desc:'Ephemeral K8s Job. Credentials in-memory only. terraform apply. outputs.json saved to S3 — read by Phase 2 OPTIMA-AI.' },
  health:          { tag:'Stage 6',              tagColor:'#D97706', tagBg:'#FEF3C7', title:'Infrastructure Health Dashboard', desc:'HealthPoller polls CloudWatch/Azure Monitor every 60s. Green/Yellow/Red signals. Azure Monitor: 1–5 min ingestion delay — staleness indicator mandatory.' },
  audit:           { tag:'Stage 7 — Phase 2',   tagColor:'#D97706', tagBg:'#FEF3C7', title:'Audit & Compliance',         desc:'24 event types. Dual-write to PostgreSQL (90 days) and S3 Object Lock WORM (7 years). Deferred to Phase 2.' },
  ...
  'optima-scan':      { tag:'OPTIMA-AI Step 1',  tagColor:'#0EA5E9', tagBg:'#E0F2FE', title:'Cost Breakdown — Provisioned LLM Stack', desc:'Detailed cost breakdown of every resource provisioned during Phase 1 Terraform execution. Resource identifiers are from outputs.json.', isOptima: true },
  ...
  'optima-savings':   { tag:'OPTIMA-AI Step 4',  tagColor:'#0EA5E9', tagBg:'#E0F2FE', title:'Savings Dashboard',         desc:'Tracks savings against Phase 1 Stage 3 approved cost baseline and the budget ceiling set in the Intake Form.', isOptima: true },
};
      ...
      <div style={{
        ...
        {meta && (
          <>
            ...
            <div style={{
              fontSize: 20, fontWeight: 600, letterSpacing: '-0.02em', marginBottom: 5,
